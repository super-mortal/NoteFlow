...
def get_openai_compatible_transcriber(provider_id=None, model_name=None):
    """
    获取 OpenAI 兼容转写器
    
    Args:
        provider_id: 供应商 ID（如 'openai', 'groq', 'custom-api'）
        model_name: 模型名称（如 'whisper-1', 'whisper-large-v3'）
    """
    # 每次都创建新实例，因为可能切换不同供应商
    provider_id = provider_id or os.environ.get("OPENAI_TRANSCRIBER_PROVIDER", "openai")
    model_name = model_name or os.environ.get("OPENAI_TRANSCRIBER_MODEL", "whisper-1")
    
    logger.info(f"创建 OpenAI 兼容转写器: provider={provider_id}, model={model_name}")
    return OpenAICompatibleTranscriber(provider_id=provider_id, model_name=model_name)

# 通用入口
def get_transcriber(transcriber_type="fast-whisper", model_size="base", device="cuda", provider_id=None, model_name=None):
    """
    获取指定类型的转录器实例
    ...
    参数:
        ...
        provider_id: OpenAI 兼容转写器的供应商 ID
        model_name: OpenAI 兼容转写器的模型名称
