...
from app.utils.path_helper import get_notes_dir, get_screenshots_dir
...
# 输出目录（统一到 data/ 下）
NOTE_OUTPUT_DIR = Path(os.getenv("NOTE_OUTPUT_DIR", str(get_notes_dir())))
NOTE_OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
IMAGE_OUTPUT_DIR = os.getenv("OUT_DIR", str(get_screenshots_dir()))
...
class NoteGenerator:
    ...
    def _init_transcriber(self) -> Transcriber:
        ...
        # 对于 openai-compatible 转写器，需要传递配置中的模型名称
        if self.transcriber_type == "openai-compatible":
            from app.services.transcriber_config_manager import TranscriberConfigManager
            config = TranscriberConfigManager().get_config()
            provider_id = config.get("openai_transcriber_provider") or None
            model_name = config.get("openai_transcriber_model") or None
            logger.info(f"OpenAI 兼容转写器配置: provider={provider_id}, model={model_name}")
