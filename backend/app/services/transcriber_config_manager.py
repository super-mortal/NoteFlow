...
class TranscriberConfigManager:
    ...
    def get_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "openai_transcriber_provider": data.get(
                "openai_transcriber_provider",
                os.getenv("OPENAI_TRANSCRIBER_PROVIDER", "openai"),
            ),
            "openai_transcriber_model": data.get(
                "openai_transcriber_model",
                os.getenv("OPENAI_TRANSCRIBER_MODEL", "whisper-1"),
            ),
            "openai_transcriber_base_url": data.get(
                "openai_transcriber_base_url",
                os.getenv("OPENAI_TRANSCRIBER_BASE_URL", ""),
            ),
            "openai_transcriber_api_key": data.get(
    ...
    def update_config(
        ...
        if openai_transcriber_provider is not None:
            data["openai_transcriber_provider"] = openai_transcriber_provider
        if openai_transcriber_model is not None:
            data["openai_transcriber_model"] = openai_transcriber_model
        if openai_transcriber_base_url is not None:
