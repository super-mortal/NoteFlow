...
class BilibiliDownloader(Downloader, ABC):
    ...
    def download(
        ...
        if self._cookiefile:
            ...
            logger.info("使用 Cookie 文件进行 B站 下载")
        else:
            logger.warning("⚠️ 未配置 B站 Cookie，可能遇到 412 错误。请在「设置 → 下载配置」中配置 Cookie")
    ...
    def download_video(
        ...
        if self._cookiefile:
            ydl_opts['cookiefile'] = self._cookiefile
            logger.info("使用 Cookie 文件进行 B站 视频下载")
        else:
            logger.warning("⚠️ 未配置 B站 Cookie，可能遇到 412 错误")
